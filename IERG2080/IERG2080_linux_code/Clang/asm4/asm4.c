...
int bin_ucp(FILE *fp, int ftype)
{
    ...
    else    // utf-16: 4 bytes version, just read as 2 bytes, invalid then skip: all half / full width char are in 0th plane
    {
        // PRTLINE;
        // get every 2 bytes
        unsigned char bytes[2];
        // fread(&bytes[0], sizeof(unsigned char), 1, fp);
        // printf("%02x\n", bytes[0]);
        int tmp = fgetc(fp);
        if(tmp == EOF)  // EOF
        {
        ...
        bytes[0] = tmp;
        bytes[1] = fgetc(fp);
        // fread(&bytes[1], sizeof(unsigned char), 1, fp);
        // PRTLINE;
        ...
        // calculate ucp
        ucp = (unsigned int)bytes[0] << 8 | (unsigned int)bytes[1];
        // printf("ftype: %d\n", ftype);
