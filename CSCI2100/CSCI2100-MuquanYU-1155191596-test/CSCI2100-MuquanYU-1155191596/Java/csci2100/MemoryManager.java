...
public class MemoryManager {
    ...
    public int request(MemoryOperation op) {
        ...
        // request with address: find the valid memMapNode or else return -1
            ...
            if(op.getAddr().compareTo(new BigInteger("0")) == -1 || op.getAddr().compareTo(TOTAL_BYTES) == 1) {return -1;}
    ...
    public boolean release(MemoryOperation op) {
        ...
        // check address in range
        if(op.getAddr().compareTo(new BigInteger("0")) == -1 || op.getAddr().compareTo(TOTAL_BYTES) == 1) {return false;}
        ...
        // if(newReleaseMode) {
            ...
            while(testingCurrEnd.compareTo(tarEndAddr) == -1) {  // currMMNend < tarEnd: not over yet
                ...
                testingCurr = testingCurr.nextNode;
                    testingCurrEnd = testingCurr.startingAddr.add(currMemMapNode.size).subtract(new BigInteger("1"));
            }
        // }
